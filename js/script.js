...
function userNameValidation(userNameValue) {
  ...
  if (!usernameRegex.test(userNameValue)) {
    console.log("Username must start with a letter, and can only contain letters, numbers, and underscores. It must be 6-20 characters long.");
    alert("Username must start with a letter, and can only contain letters, numbers, and underscores. It must be 6-20 characters long.");
  }
}
...
function validatePassword(passwordValue) {
  ...
  if (!passwordRegex.test(passwordValue)) {
    ...
    alert("Password must contain at least one digit, one lowercase, one uppercase, special character [!@#$%^&*()\-+.] and must be between 6 and 20 characters in length.       'Abcdef123!'");
  }
}
...
function validateRetypePassword(repasswordValue) {
  ...
  const repasswordRegex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()\-+.]).{6,20}$/;
...
function validateCityName(inputtxt) {
  ...
  if (!letters.test(inputtxt))
    alert("Please Type only String characters only");  
}
...
function validateZipNumber(inputnum) {
  ...
  if (!zipCodeRegex.test(inputnum))
    alert("Please input Numeric characters only and it's must be 5 numbers....");
}
